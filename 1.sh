...
echo "${YELLOW_TEXT}${BOLD_TEXT}gcloud network-connectivity hubs create ncc-hub${RESET_FORMAT}"


gcloud config set accessibility/screen_reader false
gcloud compute networks subnets list --network=vpc1-ncc

echo "${MAGENTA_TEXT}${BOLD_TEXT}Generating...${RESET_FORMAT}"
...
echo "${BLUE_TEXT}${BOLD_TEXT}[ADDRESS]..${RESET_FORMAT}"
...
echo "${YELLOW_TEXT}${BOLD_TEXT}Verify that the internal IP address ...${RESET_FORMAT}"
...
echo "${CYAN_TEXT}${BOLD_TEXT}Get the service attachment URI...${RESET_FORMAT}"
...
echo "${MAGENTA_TEXT}${BOLD_TEXT}Create the Private Service Connect...${RESET_FORMAT}"
...
--region=REGION \
--network=vpc2-ncc  \
--target-service-attachment=$ILB_IP \
...
--networks=vpc2-ncc  \
--visibility=private
echo "${MAGENTA_TEXT}${BOLD_TEXT}Add a DNS record for the Private Service Connect...${RESET_FORMAT}"

gcloud sql instances describe  \
--project=PROJECT_ID \
--format="value(dnsName)"

echo "${MAGENTA_TEXT}${BOLD_TEXT}ENTER DNS_RECORD...${RESET_FORMAT}"
read ILB_IP2

echo "ILB_IP2=$ILB_IP2"


gcloud dns record-sets create $ILB_IP2 \
--project=PROJECT_ID \
--type=A \
--rrdatas=10.2.2.70 \
--zone=cloudsql-dns
